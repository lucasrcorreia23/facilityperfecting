-- Remove FIESC vazio; garante 6 roleplays com roteiros de teste do vendedor.

-- Cliente FIESC duplicado/vazio (sem nenhum roleplay)
...
begin
  ...
  if not exists (
    select 1 from public.roleplay_readiness where client_id = fiesc_id and name = 'RP1.1 (v3) MBI'
  ) then
    insert into public.roleplay_readiness (client_id, name, persona, position, roteiro) values
      (fiesc_id, 'RP1.1 (v3) MBI', 'Carlos Mendes — Secretário de Planejamento e Inovação', 0, $fiesc_ins_rp106$## RP 1.1 — RP_106 · Carlos Mendes (Secretário de Planejamento e Inovação)
...
$fiesc_ins_rp106$);
  ...
  if not exists (
    select 1 from public.roleplay_readiness where client_id = fiesc_id and name = 'RP1.2 (v3) MBI'
  ) then
    insert into public.roleplay_readiness (client_id, name, persona, position, roteiro) values
      (fiesc_id, 'RP1.2 (v3) MBI', 'Ana Paula Silveira — Secretária de Administração e Finanças', 1, $fiesc_ins_rp107$## RP 1.2 — RP_107 · Ana Paula Silveira (Secretária de Administração e Finanças)
...
$fiesc_ins_rp107$);
  ...
  if not exists (
    select 1 from public.roleplay_readiness where client_id = fiesc_id and name = 'RP1.3 (v3) MBI'
  ) then
    insert into public.roleplay_readiness (client_id, name, persona, position, roteiro) values
      (fiesc_id, 'RP1.3 (v3) MBI', 'Roberto Nunes — Prefeito', 2, $fiesc_ins_rp108$## RP 1.3 — RP_108 · Roberto Nunes (Prefeito)
...
$fiesc_ins_rp108$);
  ...
  if not exists (
    select 1 from public.roleplay_readiness where client_id = fiesc_id and name = 'RP2.1 (v3) SENAI'
  ) then
    insert into public.roleplay_readiness (client_id, name, persona, position, roteiro) values
      (fiesc_id, 'RP2.1 (v3) SENAI', 'Laura Martins — Estudante (Ensino Médio)', 3, $fiesc_ins_rp125$## RP 2.1 — RP_125 · Laura Martins (jovem, Curso Técnico SENAI)
...
$fiesc_ins_rp125$);
  ...
  if not exists (
    select 1 from public.roleplay_readiness where client_id = fiesc_id and name = 'RP2.2 (v3) SENAI'
  ) then
    insert into public.roleplay_readiness (client_id, name, persona, position, roteiro) values
      (fiesc_id, 'RP2.2 (v3) SENAI', 'Carlos Almeida — Auxiliar de Produção', 4, $fiesc_ins_rp128$## RP 2.2 — RP_128 · Carlos Almeida (operário, Cursos Profissionais SENAI)
...
$fiesc_ins_rp128$);
  ...
  if not exists (
    select 1 from public.roleplay_readiness where client_id = fiesc_id and name = 'RP2.3 (v3) SESI'
  ) then
    insert into public.roleplay_readiness (client_id, name, persona, position, roteiro) values
      (fiesc_id, 'RP2.3 (v3) SESI', 'Mariana Silva — Profissional autônoma / mãe', 5, $fiesc_ins_rp131$## RP 2.3 — RP_131 · Mariana Silva (mãe, SESI Odonto)
...
$fiesc_upd_rp106$,
        updated_at = now()
    where client_id = fiesc_id and name = 'RP1.1 (v3) MBI';
...
$fiesc_upd_rp107$,
        updated_at = now()
    where client_id = fiesc_id and name = 'RP1.2 (v3) MBI';
...
$fiesc_upd_rp108$,
        updated_at = now()
    where client_id = fiesc_id and name = 'RP1.3 (v3) MBI';
...
$fiesc_upd_rp125$,
        updated_at = now()
    where client_id = fiesc_id and name = 'RP2.1 (v3) SENAI';
...
$fiesc_upd_rp128$,
        updated_at = now()
    where client_id = fiesc_id and name = 'RP2.2 (v3) SENAI';
...
$fiesc_upd_rp131$,
        updated_at = now()
    where client_id = fiesc_id and name = 'RP2.3 (v3) SESI';
end $$;
